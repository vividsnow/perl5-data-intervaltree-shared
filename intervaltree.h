...
#define IT_LOCK_TIMEOUT_SEC  2  /* FUTEX_WAIT timeout for stale lock detection */
...
static inline void it_recover_stale_lock(ItHandle *h, uint32_t observed_rwlock) {
    ItHeader *hdr = h->hdr;
...
static inline void it_claim_reader_slot(ItHandle *h) {
    ...
    for (uint32_t i = 0; i < IT_READER_SLOTS; i++) {
        ...
        if (__atomic_compare_exchange_n(&h->reader_slots[s].pid,
             ...
             * non-zero, and it_recover_dead_readers won't drain them
...
static inline void it_atomic_sub_cap(uint32_t *p, uint32_t sub) {
    if (!sub) return;
 ...
 * state.  it_claim_reader_slot zeros all three on every claim, so
 * leaving stale values is harmless. */
static inline void it_drain_dead_slot(ItHandle *h, uint32_t i, uint32_t pid) {
    ItHeader *hdr = h->hdr;
    ...
    if (wp)    it_atomic_sub_cap(&hdr->rwlock_waiters, wp);
    if (writp) it_atomic_sub_cap(&hdr->rwlock_writers_waiting, writp);
}
...
static inline void it_recover_dead_readers(ItHandle *h) {
    ...
    if (found_dead_reader && !any_live_reader) {
        ...
        if (cur > 0 && cur < IT_RWLOCK_WRITER_BIT) {
            ...
            for (uint32_t i = 0; !live_now && i < IT_READER_SLOTS; i++) {
        ...
        if (drain_ok) {
            for (uint32_t i = 0; i < IT_READER_SLOTS; i++) {
                uint32_t p = __atomic_load_n(&h->reader_slots[i].pid, __ATOMIC_ACQUIRE);
                if (p == 0 || it_pid_alive(p)) continue;
                it_drain_dead_slot(h, i, p);
            }
        }
    }
}
...
static inline void it_park_reader(ItHandle *h) {
...
static inline void it_unpark_reader(ItHandle *h) {
...
static inline void it_park_writer(ItHandle *h) {
    if (h->my_slot_idx != UINT32_MAX) {
        __atomic_add_fetch(&h->reader_slots[h->my_slot_idx].waiters_parked, 1, __ATOMIC_RELAXED);
...
static inline void it_unpark_writer(ItHandle *h) {
    __atomic_sub_fetch(&h->hdr->rwlock_waiters, 1, __ATOMIC_RELAXED);
...
static inline void it_reader_enter(ItHandle *h) {
    if (h->my_slot_idx != UINT32_MAX) {
...
static inline void it_reader_leave(ItHandle *h) {
...
static inline void it_rwlock_rdlock(ItHandle *h) {
    ...
    for (int spin = 0; ; spin++) {
        ...
        if (cur >= IT_RWLOCK_WRITER_BIT || cur == 0) {
 ...
 * Layout: Header -> reader_slots[1024] -> nodes[capacity] -> build_idx[capacity] */
typedef struct { uint64_t reader_slots, nodes, idx, total; } ItLayout;

static inline ItLayout it_layout_for(uint32_t capacity) {
    ItLayout L;
    L.reader_slots = sizeof(ItHeader);
    L.nodes        = L.reader_slots + (uint64_t)IT_READER_SLOTS * sizeof(ItReaderSlot);
...
static inline ItHandle *it_setup(void *base, size_t map_size,
    ...
    h->hdr          = hdr;
    h->base         = base;
    h->reader_slots = (ItReaderSlot *)((uint8_t *)base + hdr->reader_slots_off);
...
static void it_overlaps_rec(ItHandle *h, uint32_t node, int64_t ql, int64_t qh, uint32_t depth,
                            ItRes *out, uint64_t *cnt, uint64_t cap) {
    ...
    it_overlaps_rec(h, nd->left, ql, qh, depth + 1, out, cnt, cap);   /* left subtree may overlap */
    ...
    if (nd->lo <= qh)                                     /* right lo's >= node.lo; skip if node.lo > qh */
        it_overlaps_rec(h, nd->right, ql, qh, depth + 1, out, cnt, cap);
}

static uint64_t it_overlaps_locked(ItHandle *h, int64_t ql, int64_t qh, ItRes *out, uint64_t cap) {
    uint64_t cnt = 0;
    it_overlaps_rec(h, h->hdr->root, ql, qh, 0, out, &cnt, cap);
    return cnt;
}
